...
######################################## MCL-DETR Context-Guided Feature Pyramid Network start ########################################
...
######################################## MCL-DETR Context-Guided Feature Pyramid Network end ########################################
...
class EdgeEnhancer(nn.Module):
    def __init__(self, in_dim):
        super().__init__()
        # 更换为 ReLU，并加入可学习 alpha，控制增强强度
        self.out_conv = Conv(in_dim, in_dim, act=nn.ReLU())
        self.pool = nn.AvgPool2d(3, stride=1, padding=1)
        self.alpha = nn.Parameter(torch.tensor(0.1))  # 初始增强强度为0.1

    def forward(self, x):
        edge = x - self.pool(x)
        edge = self.out_conv(edge)
        return x + self.alpha * edge  # 控制增强强度，减少过激操作
...
class MutilScaleEdgeInformationEnhance(nn.Module):
    ...
    def forward(self, x):
        ...
        for idx, f in enumerate(self.features):
            feat = F.interpolate(f(x), x_size[2:], mode='bilinear', align_corners=True)
            out.append(self.ees[idx](feat))
...
######################################## MCL-DETR Edge-aware Feature Enhancement Module start ########################################
...
######################################## MCL-DETR Edge-aware Feature Enhancement Module end ########################################
...
######################################## Rethinking Transformer-Based Blind-Spot Network for Self-Supervised Image Denoising start ########################################
...
######################################## Rethinking Transformer-Based Blind-Spot Network for Self-Supervised Image Denoising end ########################################

######################################## Frequency-Spatial Entanglement Learning for Camouflaged Object Detection start ########################################
...
######################################## Frequency-Spatial Entanglement Learning for Camouflaged Object Detection end ########################################

######################################## Efficient Face Super-Resolution via Wavelet-based Feature Enhancement Network start ########################################
...
######################################## Efficient Face Super-Resolution via Wavelet-based Feature Enhancement Network end ########################################
...
######################################## Spatial-Frequency Dual Domain Attention Network For Medical Image Segmentation start ########################################
...
######################################## Spatial-Frequency Dual Domain Attention Network For Medical Image Segmentation end ########################################

######################################## Kolmogorov–Arnold Transformer start ########################################
...
######################################## Kolmogorov–Arnold Transformer end ########################################
