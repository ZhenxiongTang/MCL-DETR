...
__all__ = ['TransformerEncoderLayer_LocalWindowAttention', 'AIFI_LPE', 'TransformerEncoderLayer_DAttention', 'TransformerEncoderLayer_HiLo', 
           ...
           'TransformerEncoderLayer_TSSA', 'TransformerEncoderLayer_ASSA', 'TransformerEncoderLayer_Pola_CGLU', 'TransformerEncoderLayer_Polarity_aware_Feature_Interaction']
...
class HybridPositionalEncoding(nn.Module):
    def __init__(self, fmap_size, c1, embed_dim, alpha=0.5, trainable_temperature=False):
        super().__init__()
        self.seq_len = fmap_size
        ...
        if trainable_temperature:
            self.temperature = nn.Parameter(torch.tensor(10000.0))
        else:
            self.register_buffer('temperature', torch.tensor(10000.0))

        self.sincos_pe = AIFI_LPE.build_2d_sincos_position_embedding(
            int(fmap_size ** 0.5), int(fmap_size ** 0.5), embed_dim, self.temperature
        )

        # ✅ alpha 修改为 shape=(1, seq_len, 1)，用于每个位置的权重
        self.alpha = nn.Parameter(torch.full((1, fmap_size, 1), alpha))

    def forward(self, x):
        lpe_embed = self.lpe(x)  # shape: [1, seq_len, dim]
        sincos_embed = self.sincos_pe.to(x.device).type_as(lpe_embed)  # shape: [1, seq_len, dim]

        # ✅ 位置对齐优化（插值而非裁剪）
        if sincos_embed.shape != lpe_embed.shape:
            sincos_embed = F.interpolate(
                sincos_embed.permute(0, 2, 1),
                size=lpe_embed.size(1),
                mode='linear',
                align_corners=False
            ).permute(0, 2, 1)

        # ✅ 融合编码，alpha 为可学习张量
        fused_embed = self.alpha * lpe_embed + (1 - self.alpha) * sincos_embed
        return fused_embed


# ⚠️ 你需要定义 TransformerEncoderLayer 类，以下示意继承用
class TransformerEncoderLayer(nn.Module):
    def __init__(self, c1, cm, num_heads, dropout, act, normalize_before):
        super().__init__()
        # 示例代码，实际实现应替换为你自己的 TransformerEncoderLayer 逻辑
        self.attn = nn.MultiheadAttention(c1, num_heads, dropout=dropout, batch_first=True)
        self.norm = nn.LayerNorm(c1)
        self.ffn = nn.Sequential(nn.Linear(c1, cm), act, nn.Dropout(dropout), nn.Linear(cm, c1))
...
class AIFI_LPE(TransformerEncoderLayer):
    ...
    def __init__(self, c1, cm=2048, num_heads=8, fmap_size=20 * 20, dropout=0, act=nn.GELU(), normalize_before=False):
        ...
        self.LPE = HybridPositionalEncoding(fmap_size, c1, c1)

    def forward(self, x):
        """Forward pass for the AIFI transformer layer."""
        b, c, h, w = x.shape
        pos_embed = self.LPE(x)  # shape: [1, H*W, C]
        x = x.flatten(2).permute(0, 2, 1)  # [B, C, H, W] -> [B, H*W, C]
        x = super().forward(x, pos=pos_embed.to(x.device, dtype=x.dtype))
...
######################################## MCL-DETR Polarity-aware Feature Interaction Module start ########################################
...
class TransformerEncoderLayer_Polarity_aware_Feature_Interaction(TransformerEncoderLayer_Pola_CGLU):
        def __init__(self, c1, cm=2048, num_heads=8, dropout=0, act=nn.GELU(), normalize_before=False):
            super().__init__(c1, cm, num_heads, dropout, act, normalize_before)

            self.ffn = FMFFN(c1, cm, c1)

######################################## MCL-DETR Polarity-aware Feature Interaction Module end ########################################
